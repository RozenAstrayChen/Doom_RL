...
class AC(Policy):
    def __init__(self, map=map_health):
        ...
        self.model = ACNet(self.action_num, resolution_dim * 4).cuda()
        self.saved_actions = []
        self.rewards = []
        # a2c need s, s'
        self.s_ = []
    ...
    def update_policy(self, ):
        ...
        loss = torch.stack(
            policy_losses).mean() + torch.stack(value_losses).mean()
        loss.backward()
        self.optimizer.step()
        del self.rewards[:]
        ...
        return loss, torch.stack(value_losses).mean(), -torch.stack(
            policy_losses).mean()
    ...
    def train_model(self, load=False, num=0, iterators=1):
        ...
        for iterator in range(0, iterators):
            for epoch in range(train_episodes):
                ...
                s = self.game.get_state().screen_buffer
                s, stacked_frames = self.stack_frames(stacked_frames, s, True)
                while True:

                    action_index = self.choose_action(s)
                    ...
                    if done:
                        ...
                        n_s, stacked_frames = self.stack_frames(stacked_frames, n_s, True)

                        #s2 = None
                    else:
                        n_s = self.game.get_state().screen_buffer
